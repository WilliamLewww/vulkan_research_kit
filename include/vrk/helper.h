...
#define LOAD_INSTANCE_FUNCTION(instanceHandle, functionName,                   \
  ...
  if (functionLocalName == NULL)                                               \
    std::cerr << "Instance funtion not found: " << #functionName << std::endl;

#define LOAD_DEVICE_FUNCTION(deviceHandle, functionName, functionLocalName)    \
  PFN##_##functionName functionLocalName =                                     \
      (PFN##_##functionName)vkGetDeviceProcAddr(deviceHandle, #functionName);  \
  if (functionLocalName == NULL)                                               \
    std::cerr << "Device funtion not found: " << #functionName << std::endl;
