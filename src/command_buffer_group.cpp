...
CommandBufferGroup::CommandBufferGroup(VkDevice &deviceHandleRef,
  ...
  VkResult result =
      vkAllocateCommandBuffers(deviceHandleRef, &commandBufferAllocateInfo,
...
void CommandBufferGroup::endRecording(uint32_t commandBufferIndex) {
  VkResult result =
      vkEndCommandBuffer(this->commandBufferHandleList[commandBufferIndex]);
...
void CommandBufferGroup::submit(
    VkQueue &queueHandleRef, std::vector<SubmitInfoParam> submitInfoParamList,
    VkFence fenceHandle) {
  ...
  for (SubmitInfoParam &submitInfoParam : submitInfoParamList) {
    ...
    VkSubmitInfo submitInfo = {
        ...
        .commandBufferCount = (uint32_t)commandBufferHandleListSubmit
                                  [commandBufferHandleListSubmit.size() - 1]
                                      .size(),
        .pCommandBuffers =
            commandBufferHandleListSubmit[commandBufferHandleListSubmit.size() -
                                          1]
                .data(),
  ...
  VkResult result =
      vkQueueSubmit(queueHandleRef, (uint32_t)submitInfoList.size(),
                    submitInfoList.data(), fenceHandle);
...
void CommandBufferGroup::reset(
    uint32_t commandBufferIndex,
    VkCommandBufferResetFlags commandBufferResetFlags) {

  VkResult result =
      vkResetCommandBuffer(this->commandBufferHandleList[commandBufferIndex],
...
void CommandBufferGroup::createPipelineBarrierCmd(
  ...
  vkCmdPipelineBarrier(
      this->commandBufferHandleList[commandBufferIndex], srcPipelineStageFlags,
      dstPipelineStageFlags, dependencyFlags,
      (uint32_t)memoryBarrierList.size(), memoryBarrierList.data(),
      (uint32_t)bufferMemoryBarrierList.size(), bufferMemoryBarrierList.data(),
