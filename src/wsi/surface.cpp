...
Surface::Surface(VkInstance& instanceHandleRef,
    Platform platform,
  ...
  if (platform == Platform::Xlib) {
    ...
    VkXlibSurfaceCreateInfoKHR xlibSurfaceCreateInfo = {
      ...
      .window = *(std::static_pointer_cast<Window>
          (surfaceCreateInfoParam->windowPtr).get())
    };

    LOAD_INSTANCE_FUNCTION(instanceHandleRef, vkCreateXlibSurfaceKHR,
        pvkCreateXlibSurfaceKHR);

    VkResult result = pvkCreateXlibSurfaceKHR(instanceHandleRef,
        &xlibSurfaceCreateInfo, NULL, &this->surfaceHandle);
