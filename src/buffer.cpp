...
Buffer::Buffer(VkDevice& deviceHandleRef,
  ...
  VkResult result = vkCreateBuffer(deviceHandleRef,
      &bufferCreateInfo, NULL, &this->bufferHandle);
...
void Buffer::copyFromImageCmd(VkCommandBuffer& commandBufferHandleRef,
    VkImage& imageHandleRef,
    VkImageLayout imageLayout,
    std::vector<VkBufferImageCopy> bufferImageCopyList) {

  vkCmdCopyImageToBuffer(commandBufferHandleRef,
      imageHandleRef,
      imageLayout,
      this->bufferHandle,
      (uint32_t)bufferImageCopyList.size(),
...
void Buffer::copyFromBufferCmd(VkCommandBuffer& commandBufferHandleRef,
  ...
  vkCmdCopyBuffer(commandBufferHandleRef,
      srcBufferHandleRef,
      dstBufferHandleRef,
      bufferCopyList.size(),
      bufferCopyList.data());
...
void Buffer::mapMemory(void** hostMemory,
    VkDeviceSize offset,
    VkDeviceSize size) {
