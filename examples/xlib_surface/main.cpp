...
int main(void) {
  ...
  Surface::XlibSurfaceCreateInfoParam xlibSurfaceCreateInfoParam = {
      .displayPtr = displayPtr, .windowPtr = std::make_shared<Window>(window)};

  Surface *surface =
      new Surface(instance->getInstanceHandleRef(), Surface::Platform::Xlib,
  ...
  for (uint32_t x = 0; x < queueFamilyPropertiesList.size(); x++) {
    if (queueFamilyPropertiesList[x].queueFlags & VK_QUEUE_GRAPHICS_BIT &&
        Surface::checkPhysicalDeviceSurfaceSupport(
            activePhysicalDeviceHandle, x, surface->getSurfaceHandleRef())) {
  ...
  Swapchain *swapchain = new Swapchain(
      device->getDeviceHandleRef(), 0, surface->getSurfaceHandleRef(),
      surfaceCapabilities.minImageCount + 1, surfaceFormatList[0].format,
      surfaceFormatList[0].colorSpace, surfaceCapabilities.currentExtent, 1,
      VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_SHARING_MODE_EXCLUSIVE,
      {queueFamilyIndex}, surfaceCapabilities.currentTransform,
      VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR, presentModeList[0], VK_TRUE,
      VK_NULL_HANDLE);
  ...
  RenderPass *renderPass = new RenderPass(
      ...
      {{0, surfaceFormatList[0].format, VK_SAMPLE_COUNT_1_BIT,
  ...
  for (VkImage &imageHandle : imageHandleList) {
    imageViewList.push_back(new ImageView(
        device->getDeviceHandleRef(), imageHandle, 0, VK_IMAGE_VIEW_TYPE_2D,
        surfaceFormatList[0].format,
        {VK_COMPONENT_SWIZZLE_IDENTITY, VK_COMPONENT_SWIZZLE_IDENTITY,
         VK_COMPONENT_SWIZZLE_IDENTITY, VK_COMPONENT_SWIZZLE_IDENTITY},
        {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}));

    framebufferList.push_back(new Framebuffer(
        device->getDeviceHandleRef(), renderPass->getRenderPassHandleRef(),
        {imageViewList[imageViewList.size() - 1]->getImageViewHandleRef()},
        (VkFramebufferCreateFlags)0, surfaceCapabilities.currentExtent.width,
  ...
  Buffer *colorBuffer =
      new Buffer(device->getDeviceHandleRef(), activePhysicalDeviceHandle, 0,
                 sizeof(float) * 3, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
  ...
  while (true) {
    ...
    uint32_t currentImageIndex = swapchain->aquireNextImageIndex(
        UINT32_MAX,
        acquireImageSemaphoreList[currentFrame]->getSemaphoreHandleRef(),
        VK_NULL_HANDLE);

    commandBufferGroup->submit(
        device->getQueueHandleRef(queueFamilyIndex, 0),
        {{{acquireImageSemaphoreList[currentFrame]->getSemaphoreHandleRef()},
          {VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT},
          {currentImageIndex},
          {writeImageSemaphoreList[currentImageIndex]
               ->getSemaphoreHandleRef()}}},
        imageAvailableFenceList[currentFrame]->getFenceHandleRef());

    surface->queuePresentCmd(
        device->getQueueHandleRef(queueFamilyIndex, 0),
        {writeImageSemaphoreList[currentImageIndex]->getSemaphoreHandleRef()},
        {swapchain->getSwapchainHandleRef()}, {currentImageIndex}, NULL);
